...
class CancelAppointmentScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: FutureBuilder(
        ...
        builder: (context, snapshot) {
          ...
          return snapshot.connectionState == ConnectionState.waiting || payment == null ? const Center(child: CircularProgressIndicator(),) : Column(
              children: [
                Expanded(
                    child: SingleChildScrollView(
                      child: Column(
                        children: [
                          _warning(context, 'You are eligible for a full refund'),
                ...
                _warning(context, 'Refund will be processed within 5-7 days', isWarning: true),
                const SizedBox(height: 10,),
                _footer(context),
                const SizedBox(height: 20,),
              ],
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Icon(Icons.cancel, color: theme.primaryContainer,),
            const SizedBox(width: 10,),
            Text('Cancel Appointment', style: GoogleFonts.ubuntu(fontSize: 18, color: theme.primaryContainer),),
          ],
        ),
      ),
    );
  ...
  Widget _warning(context, String text, {bool isWarning = false}) {
    Color primary = theme.primary;
    Color border = theme.primaryContainer;
    if(isWarning){
      primary = ColorScheme.fromSeed(seedColor: const Color.fromARGB(255, 252, 223, 0), brightness: Theme.of(context).brightness).primary;
      border = ColorScheme.fromSeed(seedColor: const Color.fromARGB(255, 252, 223, 0), brightness: Theme.of(context).brightness).primaryContainer;
    }
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 10),
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10),
      ...
      child: Row(
        children: [
          Icon(isWarning ? Icons.warning : Icons.done, color: primary,),
          const SizedBox(width: 10,),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
            flex: 6,
            child: Text('Amount Refundable :', style: GoogleFonts.ubuntu(fontSize: 18, color: theme.outline),)),
          Expanded(
            flex: 4,
            child: Row(
              mainAxisAlignment: MainAxisAlignment.end,
              children: [
                Text('Rs. ${payment!.amount.toString()}', style: GoogleFonts.ubuntu(fontSize: 25, fontWeight: FontWeight.bold),),
              ],
            )),
        ],
      ),
  ...
  Widget get _heading{
    return Row(
      children: [
        ...
        Text('Payment Details', style: GoogleFonts.raleway(fontSize: 25, fontWeight: FontWeight.bold),textAlign: TextAlign.left,),
      ],
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
            flex: 4,
            child: Text(left, style: GoogleFonts.ubuntu(fontSize: 18, color: theme.outline),)),
          Expanded(
            flex: 5,
            child: Text(':  $right', style: GoogleFonts.ubuntu(fontSize: 18),)),
        ],
      ),
    );
  }
  ...
  _cancelAppointment(context) async {
    ...
    final response = await _requestRazorpayRefund();
    Navigator.of(context).pop();
    if(response['status']) {
      appointment.cancel();
      Navigator.of(context).pop();
